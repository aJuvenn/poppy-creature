...
class DiploTurnRight(LoopPrimitive):
    ...
    def __init__(self, robot, command_freq = 40):
        ...
        for i in range(4):
            leg_motors = getattr(self.robot, DiploTurnRight.leg_names[i])
            self.legs.append(DiploLeg(leg_motors, DiploTurnRight.is_front[i]))
    ...
    def setup(self):
        ...
        self.commands.append(PositionCommand(patterns[FRONT_LEFT], initial_positions[FRONT_LEFT], angle + np.pi/2. + np.pi, move_period))
        self.commands.append(PositionCommand(patterns[BACK_LEFT], initial_positions[BACK_LEFT], -angle + np.pi/2. + np.pi, move_period))
        self.commands.append(PositionCommand(patterns[BACK_RIGHT], initial_positions[BACK_RIGHT], -angle - np.pi/2. + np.pi, move_period))
        self.commands.append(PositionCommand(patterns[FRONT_RIGHT], initial_positions[FRONT_RIGHT], angle - np.pi/2. + np.pi, move_period))
